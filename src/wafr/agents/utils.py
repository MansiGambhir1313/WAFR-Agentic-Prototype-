...
def timeout_wrapper(func: Callable, timeout_seconds: float) -> Any:
    ...
    waited = 0.0
    check_interval = min(0.5, timeout_seconds / 10)  # Check every 500ms or 10% of timeout
    
    while not thread_done.is_set() and waited < timeout_seconds:
    ...
    if not thread_done.is_set() or thread.is_alive():
        logger.warning(f"Function timed out after {timeout_seconds}s - forcing failure")
...
def batch_process(
    ...
    for i in range(0, len(items), batch_size):
        ...
        def timed_processor(item, item_idx):
            try:
                return timeout_wrapper(lambda: processor(item), effective_timeout)
            except TimeoutError:
