...
def load_wafr_schema(schema_path: Optional[str] = None, use_aws_api: bool = True) -> Dict:
    ...
    if use_aws_api and _aws_schema_cache is None:
        try:
            ...
            if aws_schema and aws_schema.get('pillars'):
                ...
                logger.warning("AWS API returned empty schema, falling back to file")
    ...
    if schema_path and os.path.exists(schema_path):
        try:
            with open(schema_path, 'r', encoding='utf-8') as f:
                ...
                logger.info(f"Loaded schema from file: {schema_path}")
                return file_schema
        except Exception as e:
            logger.error(f"Error loading WAFR schema from file: {e}")
    
    logger.warning("No schema available - using empty schema")
    return {'pillars': []}
...
def _fetch_official_aws_schema() -> Optional[Dict]:
    ...
    try:
        ...
        from agents.config import BEDROCK_REGION
        from datetime import datetime
